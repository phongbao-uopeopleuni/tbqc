...
try:
    ...
    try:
        ...
        print("⚠️  Warning: Cannot import db_config, using fallback")
        def get_db_config():
            return {
...
def get_tree():
    """Get genealogy tree from root_id up to max_gen"""
    if not build_tree:
        return jsonify({'error': 'Tree functions not available'}), 500
    
    try:
        root_id = int(request.args.get('root_id', 1))
        max_gen = int(request.args.get('max_gen', 5))
    except (ValueError, TypeError):
        return jsonify({'error': 'Invalid root_id or max_gen'}), 400
    ...
    try:
        ...
        logger.error(f"Error in /api/tree: {e}")
        return jsonify({'error': str(e)}), 500
    finally:
...
@app.route('/api/ancestors/<int:person_id>', methods=['GET'])
def get_ancestors(person_id):
    """Get ancestors chain for a person"""
    if not build_ancestors_chain:
        return jsonify({'error': 'Tree functions not available'}), 500
    ...
    try:
        ...
        # Build parent map
        parent_map = build_parent_map(cursor)
        
        # Build ancestors chain
        ancestors_chain = build_ancestors_chain(person_id, persons_by_id, parent_map)
        
        # Get current person info
        person_info = persons_by_id.get(person_id, {})
        ...
        return jsonify({
            "person": {
                ...
                "common_name": person_info.get("common_name"),
                "generation_number": person_info.get("generation_number"),
...
@app.route('/api/descendants/<int:person_id>', methods=['GET'])
def get_descendants(person_id):
    """Get descendants of a person"""
    if not build_descendants:
        return jsonify({'error': 'Tree functions not available'}), 500
    ...
    try:
        ...
        # Build descendants
        descendants = build_descendants(person_id, persons_by_id, children_map, max_depth)
        
        logger.info(f"Built descendants for person_id={person_id}, max_depth={max_depth}, count={len(descendants)}")
        return jsonify({
            "root_id": person_id,
            "max_depth": max_depth,
...
def search_persons():
    ...
    try:
        generation = int(request.args.get('generation')) if request.args.get('generation') else None
    except (ValueError, TypeError):
        generation = None
    ...
    try:
        ...
        if generation:
            cursor.execute("""
                ...
                FROM persons p
                LEFT JOIN generations g ON p.generation_id = g.generation_id
                LEFT JOIN locations l ON p.origin_location_id = l.location_id
                WHERE (p.full_name LIKE %s OR p.common_name LIKE %s)
                  AND g.generation_number = %s
                ORDER BY g.generation_number, p.full_name
                LIMIT %s
            """, (search_pattern, search_pattern, generation, limit))
        else:
            cursor.execute("""
                SELECT
                    p.person_id,
        ...
        logger.info(f"Search query='{q}', generation={generation}, found={len(results)} results")
