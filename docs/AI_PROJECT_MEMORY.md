...
|-------|------------|----------------|
| **Language** | Python 3.11+ | `README.md`, `requirements.txt` |
...
| **`docs/`** | `SRS.md`, `GENEALOGY_QA_CHECKLIST.md`, rollout notes. |
...
| **`MAINTENANCE.md`** | Operational maintenance runbook: schedule, dependency updates, backup, incident response, rollback, deploy checklist. |
| **`CHANGELOG.md`** | User-facing version history (Keep a Changelog format). Update before each push to `master`. |
| **`SECURITY.md`** | Security policy: CVE tracking, fixed vulnerabilities, attack surface, anti-patterns, secret rotation. |
