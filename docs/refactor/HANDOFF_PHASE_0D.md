...
**PHẢI làm:**
- Đọc `docs/Pre-refactor May 20, 2026.md` §6.5 (Phase 0d), §22 (DB safety), §24 (Operational Readiness) — đây là plan canonical, không sửa lệch
- Đọc `docs/refactor/CHANGELOG_REFACTOR.md` để hiểu state Phase 0a/0b/0c
...
- `0e31a8d` — `[chore]` Drop bảng `facebook_tokens` (dead, 0 row). Migration note: `docs/refactor/migrations/2026-05-20_drop_facebook_tokens.md`. Đã chạy trực tiếp vào Railway prod DB.
...
docs/refactor/EXTERNAL_INTEGRATION.md      # FB API, Geoapify, RSS crawler, self-call
...
| Restore drill (§22.2) | ❌ chưa | Drill restore vào local `tbqc_drill` DB, ghi vào `docs/refactor/BACKUP_RESTORE_DRILL.md` |
...
8. **Git ignore**: `*.sql`, `folder_sql/` đều ignored. Migration scripts viết dưới dạng `.md` với SQL trong code block (xem `docs/refactor/migrations/2026-05-20_drop_facebook_tokens.md` làm template).
...
2. `docs/Pre-refactor May 20, 2026.md` §1-3, §6.5 (Phase 0d), §22, §24 (~20 phút)
3. `docs/refactor/CHANGELOG_REFACTOR.md` (~5 phút) — state tổng quan
4. `docs/refactor/BOOTSTRAP_TRUTH.md` (~3 phút) — production runtime contract
5. `docs/refactor/FROZEN_FILE_POLICY.md` (~3 phút) — what not to touch
6. `docs/refactor/migrations/2026-05-20_drop_facebook_tokens.md` (~2 phút) — migration note template
...
[docs] phase-0d step-1: ops readiness skeleton
  ...
  + docs/refactor/EXTERNAL_INTEGRATION.md (skeleton từ plan §21.3)
  + docs/refactor/BACKUP_RESTORE_DRILL.md (skeleton)
  M docs/refactor/BOOTSTRAP_TRUTH.md (Railway log retention, deploy window matrix)
  M docs/refactor/FROZEN_FILE_POLICY.md (extended URL list từ plan §21.4)
```
...
[test] phase-0d step-2: baseline measurement scripts
  ...
  + docs/refactor/baselines/baseline_thresholds.md
```
...
[docs] phase-0d step-4: backup mechanism + restore drill verified
  M docs/refactor/BACKUP_RESTORE_DRILL.md (results)
```

### Step 5: Phase 0d close (~10 phút)
```
[docs] phase-0d complete
  M docs/refactor/CHANGELOG_REFACTOR.md (status 0d ✅)
  M docs/CHANGELOG.md ([Unreleased] section)
```
...
# 7. CHANGELOG updated
grep -A2 "Unreleased" docs/CHANGELOG.md
grep "Phase 0d" docs/refactor/CHANGELOG_REFACTOR.md
```
...
- **Stuck**: đọc `docs/Pre-refactor May 20, 2026.md` §18 Failure Mode → Rollback Playbook.
