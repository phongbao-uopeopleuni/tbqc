...
def test_person_field_filtering_anonymous(flask_app):
    """Anonymous user gọi /api/persons -> KHÔNG nhận được phone/email/contact."""
    ...
    for item in items:
        ...
        assert item.get('contact') is None  # Fix F2: contact chứa SĐT — PII

def test_person_field_filtering_member(flask_app, monkeypatch):
    """Member user gọi /api/persons -> KHÔNG nhận được phone/email/contact."""
    monkeypatch.setattr(
        "auth.get_user_by_id",
